...
export function fillCombobox(element, targetValue) {
  if (!element) return false;
  const strVal = String(targetValue).trim();

  // If it's an input inside or on the combobox
  if (element instanceof HTMLInputElement) {
    return fillTextInput(element, strVal);
  }

  // If there's an associated input/select inside the container
  const innerInput = element.querySelector('input');
  if (innerInput) {
    return fillTextInput(innerInput, strVal);
  }

  const innerSelect = element.querySelector('select');
  if (innerSelect) {
    return fillSelect(innerSelect, strVal);
  }

  // Try clicking to trigger options if custom listbox
  try {
    element.click();
    setTimeout(() => {
      const options = document.querySelectorAll('[role="option"], li');
      for (const opt of options) {
        if (opt.textContent.trim().toLowerCase().includes(strVal.toLowerCase())) {
          opt.click();
          break;
        }
      }
...
export function fillField(field, targetValue) {
  ...
  switch (field.type) {
    ...
    case FIELD_TYPES.COMBOBOX:
      return fillCombobox(field.element, targetValue);
